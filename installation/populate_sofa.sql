-- Populate SOFA Table
-- This script populates the mimiciv_derived.sofa table which is required for Sepsis-3 definition.
-- WARNING: This query takes 10-30 minutes to run!
--
-- Usage: psql -d mimiciv -f populate_sofa.sql

\echo ''
\echo '============================================================'
\echo 'POPULATING SOFA TABLE'
\echo '============================================================'
\echo 'This will take 10-30 minutes...'
\echo ''

-- Truncate existing data (safer than DROP due to dependencies)
TRUNCATE TABLE mimiciv_derived.sofa;

-- Insert SOFA data
INSERT INTO mimiciv_derived.sofa
WITH co AS (
  SELECT
    ih.stay_id,
    ie.hadm_id,
  ...
  LEFT JOIN mimiciv_derived.chemistry AS chem
    ON co.hadm_id = chem.hadm_id AND co.starttime < chem.charttime AND co.endtime >= chem.charttime
  GROUP BY co.stay_id, co.hr
), plt AS (
  ...
  SELECT co.stay_id, co.hr,
    MAX(CASE WHEN uo.uo_tm_24hr >= 22 AND uo.uo_tm_24hr <= 30
        THEN CAST(uo.urineoutput_24hr AS DOUBLE PRECISION) / uo.uo_tm_24hr * 24 END) AS uo_24hr
  FROM co
  ...
  WHERE NOT epi.stay_id IS NULL OR NOT nor.stay_id IS NULL OR NOT dop.stay_id IS NULL OR NOT dob.stay_id IS NULL
  GROUP BY co.stay_id, co.hr
), scorecomp AS (
  ...
  SELECT scorecomp.*,
    ...
    CASE WHEN platelet_min < 20 THEN 4 WHEN platelet_min < 50 THEN 3 
         ...
         WHEN platelet_min IS NULL THEN NULL ELSE 0 END AS coagulation,
    CASE WHEN bilirubin_max >= 12.0 THEN 4 WHEN bilirubin_max >= 6.0 THEN 3 
         WHEN bilirubin_max >= 2.0 THEN 2 WHEN bilirubin_max >= 1.2 THEN 1 
         WHEN bilirubin_max IS NULL THEN NULL ELSE 0 END AS liver,
    CASE
    ...
    CASE WHEN gcs_min >= 13 AND gcs_min <= 14 THEN 1 WHEN gcs_min >= 10 AND gcs_min <= 12 THEN 2
         WHEN gcs_min >= 6 AND gcs_min <= 9 THEN 3 WHEN gcs_min < 6 THEN 4
         WHEN gcs_min IS NULL THEN NULL ELSE 0 END AS cns,
    CASE WHEN creatinine_max >= 5.0 THEN 4 WHEN uo_24hr < 200 THEN 4
         WHEN creatinine_max >= 3.5 AND creatinine_max < 5.0 THEN 3 WHEN uo_24hr < 500 THEN 3
         WHEN creatinine_max >= 2.0 AND creatinine_max < 3.5 THEN 2
         WHEN creatinine_max >= 1.2 AND creatinine_max < 2.0 THEN 1
         WHEN COALESCE(uo_24hr, creatinine_max) IS NULL THEN NULL ELSE 0 END AS renal
  FROM scorecomp
), score_final AS (
  SELECT s.*,
    ...
    COALESCE(MAX(respiration) OVER w, 0) + COALESCE(MAX(coagulation) OVER w, 0) + 
    COALESCE(MAX(liver) OVER w, 0) + COALESCE(MAX(cardiovascular) OVER w, 0) + 
    COALESCE(MAX(cns) OVER w, 0) + COALESCE(MAX(renal) OVER w, 0) AS sofa_24hours
  FROM scorecalc AS s
...
\echo ''
\echo '============================================================'
\echo 'SOFA TABLE POPULATED SUCCESSFULLY'
\echo '============================================================'

SELECT COUNT(*) as total_rows, COUNT(DISTINCT stay_id) as unique_stays FROM mimiciv_derived.sofa;
